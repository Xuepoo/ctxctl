# Legacy Changelog (predecessor product)

All notable changes of the **predecessor product** (carryctx-era ctxctl: SQLite
state backend, task/decision/handoff/session commands, MCP server) before the
v0.1.0 rewrite as a pure-CLI, zero-MCP, stateless context layer.

The current product's changelog lives in `CHANGELOG.md`.
